...
/// deflate structure. Slower decompression but guaranteed to work with any inflater.
...
/// Pack using standard flate2 zlib compression, ignoring the template's deflate structure.
/// The resulting container has a shorter zlib stream padded with zeros to match the template size.
pub fn pack_with_fdeflate(db: &[u8], template: &[u8], field_0x2c: u32) -> Result<Vec<u8>> {
    validate_tdb_magic(db)?;
    let header = RosterHeader::parse(template)?;
    ...
    let mut compressed_zlib = Vec::new();
    {
        ...
        enc.finish().map_err(|e| Error::Decompress(e))?;
    }
    
    // Pad with zeros to match the template's zlib payload length.
    // The game ignores the adler32 (live saves ship with 0x00000000)
    // and inflate() stops at the deflate end-of-stream marker, so trailing
    // zeros are harmless as long as the total payload fits the container.
    while compressed_zlib.len() < template_zlib_len {
        compressed_zlib.push(0);
    }
    compressed_zlib.truncate(template_zlib_len);
    
    let mut out = Vec::with_capacity(XBOX360_HEADER_SIZE + compressed_zlib.len());
    out.extend_from_slice(&template[..XBOX360_HEADER_SIZE]);
    ...
    debug_assert_eq!(out.len(), XBOX360_HEADER_SIZE);
    out.extend_from_slice(&compressed_zlib);
